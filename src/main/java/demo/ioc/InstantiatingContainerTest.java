...
public class InstantiatingContainerTest {
    ...
    public void testXmlBeanFactoryBaseOnFileSystem() {
        ...
        File file = new File("src/main/resources/fileSystemConfig.xml");
    ...
    public void testXmlBeanFactoryBaseOnClassPath() {
        ...
        Resource resource = new ClassPathResource("ioc/classpath.xml");
        //2.初始化容器
        BeanFactory beanFactory = new XmlBeanFactory(resource);
        //2、从容器中获取Bean
        HelloApi helloApi = beanFactory.getBean("hello", HelloApi.class);
    ...
    public void testClassPathXmlApplicationContext() {
        //1.准备配置文件，从当前类加载路径中获取配置文件
        //2.初始化容器
        BeanFactory beanFactory = new ClassPathXmlApplicationContext("ioc/classpath.xml");
        //2、从容器中获取Bean
        HelloApi helloApi = beanFactory.getBean("hello", HelloApi.class);
        //3、执行业务逻辑
        helloApi.sayHello();
    ...
    public void testFileSystemApplicationContext() {
        //1.准备配置文件，从文件系统获取配置文件，默认是相对路径，可以指定绝对路径
        //2.初始化容器
        BeanFactory beanFactory = new FileSystemXmlApplicationContext("src/main/resources/fileSystemConfig.xml");
        //2、从容器中获取Bean
        HelloApi helloApi = beanFactory.getBean("hello", HelloApi.class);
        //3、执行业务逻辑
        helloApi.sayHello();
